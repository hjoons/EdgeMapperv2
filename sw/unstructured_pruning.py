...
if __name__ == "__main__":
    ...
    unstructured_prune_vals = [0, 0.5, 0.6, 0.7, 0.8, 0.9, 0.95]

    for value in unstructured_prune_vals:
        ...
        # Check if the module has a 'weight' attribute
            if hasattr(module, 'weight'):
                ...
                prune.l1_unstructured(module, name='weight', amount=value)
        print(f"Training for pruning: {value}")
        print("---------------------------------")
        train(model, train_loader, logger)
        test_model(model, eval_loader, logger, value)
        print(f"Evaluation for pruning: {value}")
        print("---------------------------------")
        evaluate(model, eval_loader)
        print("Saving model...")
        torch.save(model.state_dict(), f'pruned_model_{value}.pth')
        print("Model saved.")
        print()
        # break
