...
def mobilevit_xxs():
    ...
    return MobileViT((480, 640), dims, channels, expansion=2), enc_type
...
def mobilevit_xs():
    ...
    return MobileViT((480, 640), dims, channels), enc_type
...
class UpSample_layer(nn.Module):
    def __init__(self, inp, oup, flag, sep_conv_filters, name):
...
class build_METER_model(nn.Module):
    def __init__(self, arch_type):
        ...
        self.decoder = decoder(typ=enc_type)
...
if __name__ == "__main__":
    sample_input = torch.rand(1, 3, 480, 640)
    model = build_METER_model('s')
    model(sample_input)
    print("Hello World")
